...
parser.add_argument('--fusion_mode', type=str, default="concat", 
                    ...
                    help="多序列融合模式: 'concat' 为原始通道拼接, 'transformer' 为基于Transformer的像素级加权融合")
# Transformer 融合模块配置参数
parser.add_argument('--transformer_embed_dim', type=int, default=24, help="Transformer嵌入维度（推荐24）")
parser.add_argument('--transformer_num_heads', type=int, default=2, help="Transformer注意力头数")
parser.add_argument('--transformer_num_layers', type=int, default=1, help="Transformer层数")
parser.add_argument('--transformer_dropout', type=float, default=0.1, help="Transformer dropout概率")
parser.add_argument('--transformer_spatial_pool_size', type=int, default=128, help="空间池化尺寸（平衡计算量和细节）")
parser.add_argument('--dir_suffix', type=str, default='', help='实验目录后缀，如 -2 则目录变为 xxx_fold5-1-2')
parser.add_argument('--pretrained_ckpt', type=str, default=None, help="预训练模型checkpoint路径，用于继续训练或迁移学习")
parser.add_argument('--train_batch_size', type=int, default=4)  # 训练的batch_size，根据显存调整
parser.add_argument('--num_samples', type=int, default=6)  # 一个病人的batch（最终batch size 等于训练batch size×num_samples）
parser.add_argument('--val_batch_size', type=int, default=2)  # 测试batch_size，根据显存调整
parser.add_argument('--val_num_samples', type=int, default=4)  # 测试时一个病人的batch（最终batch size 等于训练batch size×num_samples）
parser.add_argument('--accumulation_steps', type=int, default=1, help='梯度累积步数，用于模拟更大batch size（显存不足时设置为2或4）')
...
parser.add_argument('--pool_size', type=int, default=50)  # 图像池的缓存数
# todo 这一句是不能用命令行执行的
parser.add_argument('--loss_weight_dict', type=dict, default={'ls_GAN_loss': 8, 'L1_loss': 25, 'SSIM_loss': 20},
...
parser.add_argument('--num_workers', type=int, default=4, help='DataLoader worker数 若卡住请设为0')
...
parser.add_argument('--lr', type=float, default=2e-5)  # 初始or最大学习率
parser.add_argument('--lr_low', type=float, default=1e-7)  # 最小学习率,设置为None,则为最大学习率的1e+6分之一(不可设置为0)
parser.add_argument('--num_epochs', type=int, default=200)  # 总epoch
parser.add_argument('--lr_cos_epoch', type=int, default=200)  # cos退火的epoch数,一般就是总epoch数-warmup的数,为0或False则代表不使用
...
parser.add_argument('--cuda_idx_list', type=int, nargs='+', default=[0,1,2,3], help='使用哪些GPU，例如 0 1 2 表示使用 0,1,2 号卡')
