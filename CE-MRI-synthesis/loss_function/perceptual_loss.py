...
class PerceptualLoss(nn.Module):
    ...
    def __init__(
        ...
        if spatial_dims == 3 and is_fake_3d is False:
            ...
            self.perceptual_function = RadImageNetPerceptualSimilarity(net=network_type, verbose=True)
...
class RadImageNetPerceptualSimilarity(nn.Module):
    ...
    def __init__(self, net: str = "radimagenet_resnet50", verbose: bool = False) -> None:
        super().__init__()
        self.model = torch.hub.load("/home/medai05/.cache/torch/hub/Warvito_radimagenet-models_main", source='local',
                                    model=net, verbose=verbose, )
        self.eval()
